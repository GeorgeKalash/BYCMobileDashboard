  ...
  const displayAlert = () => {
    SweetAlert.fire({ text: "Please Click on this button it's big surprise for you.",confirmButtonColor:"#7A70BA" }).then((result) => {
      if (result.isConfirmed) {
        SweetAlert.fire({ text: "Thank you for visit BYC : true" ,confirmButtonColor:"#7A70BA"});
      } else {
        SweetAlert.fire({ text: "Thank you for visit BYC : null",confirmButtonColor:"#7A70BA" });
      }
    });
  };
