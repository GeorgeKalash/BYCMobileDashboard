  ...
  async (_, { rejectWithValue }) => {
    try {
      const response = await axios.get(
        `${process.env.NEXT_PUBLIC_AuthURL}/MA.asmx/getAC?_accountName=${process.env.NEXT_PUBLIC_ACCOUNT_NAME}`
      );
...
export const login = createAsyncThunk(
  ...
  async (
    ...
    try {
      ...
      const getUS2 = await axios.get(
        `${getAC.api}/SY.asmx/getUS2?_email=${username}`,
        {
          headers: {
      ...
      const signIn3Params = `_email=${username}&_password=${encryptePWD(
        password
      )}&_accountId=${getAC.accountId}&_userId=${getUS2.data.record.recordId}`;

      const signIn3 = await axios.get(
        `${process.env.NEXT_PUBLIC_AuthURL}/MA.asmx/signIn3?${signIn3Params}`,
        {
          headers: {
            accountId: JSON.parse(getAC.accountId),
            dbe: JSON.parse(getAC.dbe),
